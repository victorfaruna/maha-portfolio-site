...
export function ArticleShareBar({
  ...
  const [displayViews, setDisplayViews] = useState(viewsCount);

  useEffect(() => {
    if (!articleId) return;
    const storageKey = `article_views_inc_${articleId}`;
    const hasIncrementedSession = sessionStorage.getItem(storageKey);

    // Get current stored local views or fallback to viewsCount prop
    const localStoreKey = `article_views_total_${articleId}`;
    const storedTotal = localStorage.getItem(localStoreKey);
    let currentTotal = storedTotal ? parseInt(storedTotal, 10) : viewsCount;

    if (!hasIncrementedSession) {
      currentTotal += 1;
      sessionStorage.setItem(storageKey, "true");
      localStorage.setItem(localStoreKey, currentTotal.toString());
    }

    setDisplayViews(currentTotal);
  }, [articleId, viewsCount]);
  ...
  if (shareOnly) {
    return (
      <div className="my-8 py-6 border-y border-border/70 flex flex-col sm:flex-row items-center justify-between gap-4 bg-secondary/20 px-6 rounded-2xl">
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          ...
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold rounded-full border transition-all duration-300 ${
              copied
                ...
                : "bg-white text-brand-navy border-border hover:bg-brand-pink/10 hover:border-brand-pink/30 hover:text-brand-pink shadow-sm"
            }`}
          ...
          <button
            ...
            className="p-2.5 rounded-full bg-white text-brand-navy border border-border hover:bg-[#0A66C2] hover:text-white hover:border-[#0A66C2] transition-all duration-300 flex items-center justify-center shadow-sm"
          ...
          <button
            ...
            className="p-2.5 rounded-full bg-white text-brand-navy border border-border hover:bg-black hover:text-white hover:border-black transition-all duration-300 flex items-center justify-center shadow-sm"
          ...
          <button
            ...
            className="p-2.5 rounded-full bg-white text-brand-navy border border-border hover:bg-[#1877F2] hover:text-white hover:border-[#1877F2] transition-all duration-300 flex items-center justify-center shadow-sm"
          ...
          <button
            ...
            className="p-2.5 rounded-full bg-white text-brand-navy border border-border hover:bg-[#25D366] hover:text-white hover:border-[#25D366] transition-all duration-300 flex items-center justify-center shadow-sm"
